...
const DB_PATH = process.env.SHOP_DB || path.join(__dirname, '..', '..', 'shop.db');
...
db.exec(`
  ...
  CREATE TABLE IF NOT EXISTS products (
    ...
    supplier         TEXT,
    fresh_life_days  INTEGER NOT NULL DEFAULT 2,   -- delivered fresh, never frozen
    frozen_life_days INTEGER NOT NULL DEFAULT 60,  -- how long it keeps in the freezer
    thaw_hours       INTEGER NOT NULL DEFAULT 12,  -- freezer -> ready to case
    floor_life_days  INTEGER NOT NULL DEFAULT 1,   -- 0 = sell the same day it hits the floor
    par_level        REAL,
    notes            TEXT,
    active           INTEGER NOT NULL DEFAULT 1
  );
  ...
  CREATE TABLE IF NOT EXISTS batches (
    ...
    lot_code        TEXT,
    qty             REAL NOT NULL,
    state           TEXT NOT NULL,                 -- delivered|frozen|thawing|floor|sold_out|discarded
    location        TEXT NOT NULL DEFAULT 'shop',  -- shop | trailer
    delivered_on    TEXT,
    frozen_on       TEXT,
    thaw_started_at TEXT,
    thaw_ready_at   TEXT,
    floor_on        TEXT,
    discard_by      TEXT,                          -- YYYY-MM-DD, recomputed on each move
    closed_at       TEXT,
    supplier        TEXT,
    notes           TEXT,
  ...
  CREATE TABLE IF NOT EXISTS task_templates (
    id           INTEGER PRIMARY KEY AUTOINCREMENT,
    shift        TEXT NOT NULL,                    -- opening | mid | closing
    title        TEXT NOT NULL,
    area         TEXT,
  ...
  CREATE TABLE IF NOT EXISTS shifts (
    ...
    cover_status     TEXT,                          -- NULL | requested | claimed
...
// ── Activity log ──────────────────────────────────────────────────────────────
function log(employeeId, entity, entityId, action, detail = null) {
  db.prepare(`INSERT INTO activity_log (employee_id,entity,entity_id,action,detail)
              VALUES (?,?,?,?,?)`).run(employeeId ?? null, entity, entityId ?? null, action, detail);
}

// ── Seed data ─────────────────────────────────────────────────────────────────
// Runs once on an empty database. Everything here is editable in the Admin tab.
const SEED_PRODUCTS = [
...
function seedIfEmpty() {
  ...
  const insertProduct = db.prepare(`INSERT OR IGNORE INTO products
    ...
    VALUES (?,?,?,?,?,?,?)`);
  const insertTask = db.prepare(`INSERT INTO task_templates (shift,title,area,sort_order)
    VALUES (?,?,?,?)`);

  db.transaction(() => {
    for (const p of SEED_PRODUCTS) insertProduct.run(...p);
    for (const [shift, list] of Object.entries(SEED_TASKS)) {
      list.forEach(([title, area], i) => insertTask.run(shift, title, area, i * 10));
    }
...
module.exports = { db, getSetting, setSetting, allSettings, log, DB_PATH };
